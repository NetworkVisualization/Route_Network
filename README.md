...
项目展示
![](E:\MarkDown\文档\picture\6b7febbbef64b39e0a80bcba6afd0c4.png)
...
| assets              | 静态资源目录，放置 logo 、背景图片、实验图片                                 |
...
| components 详解               | 作用/功能                                        |
| ----------------------------- | ------------------------------------------------ |
| center/centerMap              | 中心世界地图，带有时间轴                         |
| centerLeft/centerLeft1Chart   | 左上方双折线图，动态（根据国家进行改变）         |
| centerRight/centerRightChart  | 右边第一个雷达图，动态（根据国家进行改变）       |
| centerRight/centerRightChart2 | 右边第二个图片库，存放研究分析的图（可放大）     |
| centerRight/centerRightChart3 | 右边第三个折线柱状图，静态（不用跟随时间轴变化） |
...
安装完依赖包之后然后使用 `vue-cli` 或者直接使用命令`npm run serve`，就可以启动项目，启动项目后需要手动全屏（按 F11）。

如果编译项目的时候提示没有 DataV 框架的依赖，输入 `npm install @jiaminghi/data-view` 或者 `yarn add @jiaminghi/data-view` 进行手动安装。
